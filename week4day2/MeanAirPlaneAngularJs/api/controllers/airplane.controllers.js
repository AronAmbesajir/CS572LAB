...
module.exports.airplaneGetAll = function (req, res) {
    ...
    // using promise
    Airplane.find().skip(offset).limit(count).exec().then(airplanes=>{
        response.message=airplanes;
        if(!airplanes){
            response.status=400;
...
//get one airplane  using promise
module.exports.airplaneGetOne = function (req, res) {
    ...
    Airplane.findById(airId).exec().then(airplane=>{
...
module.exports.airplaneSearch = function (req, res) {
    const modelNum = req.params.modelNum;
    Airplane.find({"modelNum":modelNum}).exec(function (err, airplane) {
        const response={
            status:200,
            message:airplane
        }
        ...
        console.log("Found Airplane by model ");
        res.status(response.status).json(response.message);
    });
}